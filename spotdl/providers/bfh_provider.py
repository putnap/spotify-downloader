...
def search_and_get_best_match(
    ...
    if isrc is not None:
        isrc_results = _query_and_simplify(isrc)
    ...
    song_results = _query_and_simplify(song_title)

    # Order results
    songs = _order_bfh_results(
...
def _map_result_to_song_data(result: ResultSet) -> dict:
    url = result.find('a', attrs={'class': 'playlist-down'})['href']
    for i in range(5):
        song_page = requests.get('https://biffhard.click' + url)
    ...
    try:
        download_url = soup.find('a', {"id": "SongView"})['href']
    except:
        return {}

    song_data = {
        "name": result.find('span', attrs={'class': 'playlist-name-title'}).find('a').text,
        "type": "song",
        "artist": result.find('span', attrs={'class': 'playlist-name-artist'}).find('a').text,
        "length": _parse_duration(result.find('span', attrs={'class': 'playlist-duration'}).text),
        "link": download_url,
        "position": 0,
    }
...
def _query_and_simplify(search_term: str) -> List[dict]:
    """
    ...
    url = "https://biffhard.click/api/search?query=" + urllib.parse.quote_plus(search_term)
    for i in range(5):
        search_response = requests.get(url)
        if (search_response.status_code == 200):
            soup = BeautifulSoup(search_response.content, 'html.parser')
            items = soup.find('ul', attrs={'class': 'playlist'}).find_all('li')

            return list(map(_map_result_to_song_data, items))
